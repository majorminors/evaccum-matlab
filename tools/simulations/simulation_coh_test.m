%% Simulate a coherence threshold test for evidence accumulation task
% Dorian Minors
% Created: OCT19
% Last Edit: 17OCT19
...
p.training_coh_lvl_mid = 0.60; % what coherence level for mid (training level 2_
...
p.num_trials_per_block = 16; % *p.stim_mat* - must be divisible by p.num_cues && >=15*p.num_points
...
try
    ...
    for block = 1:p.act_block_num
        ...
        while i < p.act_trial_num
            ...
            % code response info
            t.firstPress = KbName(p.resp_keys{1}); % response a/s if odd participant number
            ...
            resp = idealObserver(p.stim_mat(i,6),points); % simulate response 1 0;
