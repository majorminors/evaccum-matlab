function [remove,weights,TraMat,temcor,spacor,varexpl,ICs_ch,ICs_EMG,sphere] = detect_ICA_artefacts_v2_PD(S)
...
try TemPval    = S.TemPval;     catch; TemPval    =0.05;end
...
if isempty(refs.tem) 
    ...
    elseif length(refs.tem) ~= length(refs.spa)           
        refs.spa{4} = []; % no spatial template for EMG
    end
end
...
if size(FiltPars,2) == 3
    fprintf('Bandpass filtering from %d to %d Hz (warning - can fail)\n',FiltPars(1), FiltPars(2));
    for r=1:Nrefs%EMG filtered on a different band
        refs.tem{r} = ft_preproc_bandpassfilter(refs.tem{r}, FiltPars(1,3), FiltPars(r,1:2),  [], 'but','twopass','reduce');
    end
    if ~isempty(regexp(S.t_ref_labs{r},'EMG','match'))
        ICs_EMG  = ft_preproc_bandpassfilter(ICs,  FiltPars(1,3), FiltPars(r,1:2),  [], 'but','twopass','reduce')';
    end
        ICs_ch  = ft_preproc_bandpassfilter(ICs,  FiltPars(1,3), FiltPars(1,1:2),  [], 'but','twopass','reduce')';
    
    
elseif size(FiltPars,2) == 2
...
for r = 1:Nrefs
    ...
    if ~isempty(refs.tem{r})
        ...
        %different filter settings for EMG and other refs
        if ~isempty(regexp(S.t_ref_labs{r},'EMG','match'));IC_d = ICs_EMG;else; IC_d = ICs_ch;end
        
        for k = 1:PCA_dim
            [temcor(r,k),tempval(r,k)] = corr(refs.tem{r}',IC_d(:,k));
        end
...
if ~exist('ICs_EMG','var');ICs_EMG=[];end
