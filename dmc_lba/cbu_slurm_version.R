...
data <- read.csv(here::here("/imaging/woolgar/projects/Dorian/evaccum/evaccum-matlab/data/meg_pilot_4/behavioural/DMC_table.csv"),na.strings=c("","NA")) %>% as.data.frame()
...
LBAFit <- function(i,data) {
  ...
  save(sampling_LBA, file = here::here('jobs/hierarchical/',savename))
  ...
  save(hsampling_LBA, file = here::here('jobs/hierarchical/',savename))
