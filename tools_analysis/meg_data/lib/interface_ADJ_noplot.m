...
% LEH lines 102 and 142 commented out
% Usage:
...
if size(EEG.data,3)<=1 % epochs must be extracted
        ...
        if si==0 %add events
            ...
            EEG=eeg_checkset(EEG,'eventconsistency'); %LEH removes acct
        end
        
        EEGep = pop_epoch( EEG, {  '5sec'  }, [0 lag], 'newname', [EEG.setname '_ep5'] , 'epochinfo', 'yes');
%         % removing baseline
%         EEGep = pop_rmbase( EEGep, [0  0]);
        %EEGep = eeg_checkset(EEGep); LEH
        
    %%collects ICA data from EEG LEH commented out
...
%     end;
   ...
   % run ADJUST
   EEG.icawinv = pinv(EEG.icaweights); % DP added it
   [art, horiz, vert, blink, disc,...
        soglia_DV, diff_var, soglia_K, med2_K, meanK, soglia_SED, med2_SED, SED, soglia_SAD, med2_SAD, SAD, ...
...
%         disp('Warning: EEG.icaact missing! Recomputed from EEG.icaweights, EEG.icasphere and EEG.data');
%         % Next instruction: see eeg_checkset
%         EEG.icaact = reshape(EEG.icaweights*EEG.icasphere*reshape(EEG.data,...
%             [70 size(EEG.data,2)*size(EEG.data,3)]),...
%             [60 size(EEG.data,2) size(EEG.data,3)]);
% %         EEG.icaact = reshape(EEG.icaweights*EEG.icasphere ...
% %              *reshape(EEG.data(1:size(EEG.icaweights,1),:,:),[size(EEG.icaweights,1) size(EEG.data,2)*size(EEG.data,3)]), ...
% %               [size(EEG.icaweights,1) size(EEG.data,2) size(EEG.data,3)]);
%    
%     
%     end;
% 
