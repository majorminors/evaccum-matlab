...
if ~settings.ctr
    ...
    ICA.FiltPars = [repmat(ICA.FiltPars,[3,1]);FiltPars_EMG];
else
...
for m = toDoW %startm:length(modalities)
    ...
    if m==3 && ~settings.ctr%EEG does not have space templates -> eeg artefact rejection done below using adjust
        ICA.refs.spa = {};
        tmp = ICA.refs.tem{4};
        ICA.refs.tem   = {};
        ICA.refs.tem{1}= tmp;clear tmp;
        ICA.s_ref_labs = {};
        ICA.t_ref_labs = {};
        ICA.t_ref_labs{1} = t_ref_chans{4};
        ICA.FiltPars = [2  8 D.fsample];
