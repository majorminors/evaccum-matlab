...
% Model fitting
Model_Feature=[1 4];%select the free parameters for the model (e.g. std across fingers and accumulation rate)
numParam=getModelParam_fixC0_2afc(Model_Feature);
parange=[zeros(1,numParam);zeros(1,numParam)+10]'; % the range (min max) within which searching for optimal value (e.g. 0 and 10 for T0 would be don't go below 0 secs or above 10 secs - currently arbitrary - keeps the model from trying insensible values)
