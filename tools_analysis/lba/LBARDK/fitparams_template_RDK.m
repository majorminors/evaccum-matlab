function bestpar=fitparams_template_RDK(errfunc,startpar,mod_feature,data2fit,paramconstrains,bayesian)
...
%  Bayesian ->1, 0 (fminsearch) implements Bayesian Adaptive Direct Search
%                   for fitting via maximum likelihood estimation
%
% Outputs:
...
if ~ bayesian
    ...
    options.Display='iter';
    options.MaxFunEvals = 10000;
    options.UncertaintyHandling = 1; %obective is stochastic
    bestpar=bads(errfunc,bestpar(:)',paramconstrains(:,1)',paramconstrains(:,2)',paramconstrains(:,1)',paramconstrains(:,2)',[],options,mod_feature,goalstat);
end
end
