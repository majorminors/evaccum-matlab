function a2_megTriggers(thisSubject,datadir,toolsdir,overwrite)
% this will produce a trl file as spm and osl expect
% it will also create a new behavioural file which includes our behavioural
% file from the experiment and adds to it the (more precise?) meg trigger
% timings
% for fieldtrip, you need to import this file as cfg.trls for
% ft_definetrials, but the format is essentially the same where:
% col 1: start time
% col 2: end time
% col 3: offset from start time of trial/stimulus onset
% here we save condition labels as a separate variable (as spm and osl
% prefer)
% fieldtrip expects additional trial book keeping to be imported into the
% trialinfo variable, so you'd need to convert these to codes e.g.:
% cond1 = 1, cond2 = 2 and so on appended as col 4
% you can do all this with the custom function option you can pass to ft_definetrial
...
% I'm pretty sure I only use d (where I once saved all 'data' when I was overly clever with my coding) from this, but I also want to grab something
% from p (my 'parameters').
fixationTimings = p.fixation_dots_duration_vector;
...
%     megRTs = megRTs+34;
    ...
    if size(block,1) ~= numel(trialOnsets)
        ...
        switch thisSubject.id
            ...
            case 'S10'
                if runi == 1 % extra photodiodes, so delete them
            ...
            case 'S11'
                if runi == 4 % extra photodiodes, so delete them
                    megRTs = megRTs(1:size(block,1));
                    trialOnsets = trialOnsets(1:size(block,1));
                    responseTimes = responseTimes(1:size(block,1));
                    edited = 1; 
            ...
            case 'S15'
                if runi == 2
                    ...
                    % initially just made these zeros, but fieldtrip
                    % doesn't like a trial onset that begins before data
                    % exists, so I'm hardcoding in the offset from the
                    % epoching time window here
                    % this means I'm relying on the rt being 0 to filter
                    % these out later
                    trialOnsets = [zeros(1,tmp)';trialOnsets]+1501;
                    responseTimes = [zeros(1,tmp),responseTimes]+1501;
                    edited = 1;
                    skipCorr = 1;
                elseif runi == 5 % extra photodiodes, so delete them
                    megRTs = megRTs(1:size(block,1));
                    trialOnsets = trialOnsets(1:size(block,1));
                    responseTimes = responseTimes(1:size(block,1));
                    edited = 1;
            ...
            case 'S52'
                if runi == 1 % this correlation between behavrt and megrt is almost 0; it's a shit run anyway so let's ignore it and just use the others
                    megRTs(:) = 0;
                    skipCorr = 1;
                end
        end
    ...
    % effects in freqs like alpha and beta
    % that said, I will just set this to the trial onset and offset and edit it
    % later when I read it in (e.g. fieldtrip has an option to set the
    % trialfunction that could read this in and alter it to suit the
    % analysis)
    trialWindow = [0 1500];%in samples at 1000Hz sampling % what time window 1500 = 1.5s
    ...
    trl(:,3) = trialWindow(1);% add info about what the onset offset is
    alltrl = cat(1,alltrl,trl); % we might want this later too
...
megBehaviouralData(:,end+1) = repmat(fixationTimings(1:64),size(megBehaviouralData,1)/64,1);
