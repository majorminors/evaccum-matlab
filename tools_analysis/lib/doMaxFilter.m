...
%% 1. do SSS filtering
% this is "Signal-Space Separation" (SSS): essentially separating magnetic
% signals coming from within some sphere under the sensors (i.e. a head)
% from those from outside a sphere surrounding the sensors
...
for i = nfiles
    ...
    % people don't seem to like using this---it apparently can damage the
    % data, especially when compensating for large movements (ohba has
    % something on this)
    % but I guess we'd use this if we were doing a group analysis or
    % something? This will arrange the head positions to a default so more
